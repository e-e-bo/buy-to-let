...
g inv_oa = inv == 1 & inlist(substr(bokomnr,1,2),"02","03") // Oslo and Akershus
...
sum inv_oa if bokomnr!=""

* Time-dimension of the share (added 03.07.2023)
collapse inv*, by(month)
g invsh_o = inv_o / inv
g invsh_oo = inv_oo / inv
g invsh_oa = inv_oa / inv

format month %tm
line invsh_o month || line invsh_oo month || line invsh_oa month || if month < 645, graphregion(fcolor(white)) scheme(s2mono) // (data on bokomnr limited at the end of 2013, and not existing afterwards)
line invsh_o month || line invsh_oo month || line invsh_oa month || if month < 636, graphregion(fcolor(white)) scheme(s2mono) // (data on bokomnr last covers 31.12.2012)

g year = year(dofm(month))

table year if year < 2013, stat(mean invsh_o) 
table year if year < 2013, stat(mean invsh_oo)
table year if year < 2013, stat(mean invsh_oa)
